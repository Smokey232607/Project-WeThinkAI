...
def main():
    ...
    # Cleanup command (NEW - marks old overdue tasks as abandoned)
    cleanup_parser = subparsers.add_parser("cleanup", help="Mark very old overdue low-priority tasks as abandoned")
    ...
    if args.command == "create":
        ...
        if task_manager.export_to_csv(args.filename):
            print(f"Exported all tasks to '{args.filename}'")
        else:
            print("Failed to export tasks.")

    elif args.command == "cleanup":
        count = task_manager.process_abandoned_tasks()
        if count > 0:
            print(f"Marked {count} old overdue tasks as abandoned.")
        else:
            print("No tasks needed to be abandoned.")
